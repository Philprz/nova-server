...
# Prompt systeme commun a extract_quote_info (extrait pour reutilisation par LLMRouter)
_QUOTE_INFO_SYSTEM_PROMPT = """
...
class LLMExtractor:
    ...
    # === DEPRECATED v2.4 (migration vers LLMRouter) — kept commented for rollback ===
    # Remplaces par get_llm_router().call(system_prompt, user_message).
    # A supprimer apres stabilisation en production.
    # @retry(
    ...
    async def _call_claude_DEPRECATED(self, message: str):
...
"""
    ...
    async def _call_openai_DEPRECATED(self, message: str):
        """Fallback à OpenAI. DEPRECATED v2.4 — migre vers LLMRouter."""
    ...
    async def extract_quote_info(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            llm_text = await get_llm_router().call(
                system_prompt=_QUOTE_INFO_SYSTEM_PROMPT,
                user_message=user_message,
                max_tokens=1024,
                temperature=0.0,
            )
        except Exception as e:
            logger.error(f"LLMRouter: toute la chaine a echoue pour extract_quote_info: {e}")
            return {"error": f"LLM indisponible: {e}"}

        logger.info(f"CONTENU LLM EXTRAIT: {llm_text}")
        start_idx = llm_text.find("{")
        end_idx = llm_text.rfind("}") + 1
        if start_idx < 0 or end_idx <= start_idx:
            logger.error("Impossible de trouver du JSON dans la réponse LLM")
        ...
        try:
            ...
            logger.error(f"JSON LLM invalide: {e}")
            return {"error": "JSON invalide dans la reponse LLM"}
    ...
    """
        ...
        try:
            response = await get_llm_router().call(
            ...
            if extracted:
                return extracted
            logger.warning("Pas de JSON dans reponse LLM pour critères produit")
    ...
    """
            ...
            content = await get_llm_router().call(
                system_prompt=system_prompt,
                user_message=user_message,
                max_tokens=1024,
                temperature=0.0,
            ...
            logger.warning("JSON non valide dans la réponse LLM")
