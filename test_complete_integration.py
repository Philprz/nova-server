#!/usr/bin/env python3
"""
Test complet de l'intégration du workflow de devis dans l'assistant intelligent
"""
...
def test_workflow_integration():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('quote_data'):
                ...
                print(f"   Données devis disponibles: {bool(quote_data)}")
    ...
    print("\n[3] Test des suggestions...")
    try:
        response = requests.get(f"{ASSISTANT_URL}/suggestions/quote")
        if response.status_code == 200:
            data = response.json()
            print(f"[OK] Suggestions OK: {len(data.get('suggestions', []))} suggestions")
        else:
            print(f"[ERREUR] Suggestions echouees: {response.status_code}")
    except Exception as e:
        print(f"[ERREUR] Erreur suggestions: {e}")
    
    # Test 4: Interface HTML
...
def test_error_handling():
    ...
    print("\n[ERREURS] Test de la gestion d'erreurs")
    print("=" * 35)
